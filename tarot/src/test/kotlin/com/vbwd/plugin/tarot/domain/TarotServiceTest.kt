...
class TarotServiceTest {
    ...
    @Test
    fun `fetchDailyLimits returns the limits on success`() = runTest {
    ...
    @Test
    fun `fetchDailyLimits surfaces a failure (success=false) instead of a false success`() = runTest {
        coEvery { client.request<LimitsResponse>(HttpMethod.GET, "/taro/limits", any(), any()) } returns
    ...
    @Test
    fun `submitSituation returns the interpretation`() = runTest {
        coEvery {
